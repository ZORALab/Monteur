...
// Run is to execute the tester algorithm.
func (fx *tester) Run() (statusCode int) {
	err := fx._init()
	if err != nil {
	...
	// parse all testers
	err = filepath.Walk(fx.workspace.Filesystem.TestConfigDir,
		fx._filterTester)
	if err != nil {
		return fx._reportError(err)
	}
...
func (fx *tester) _filterTester(path string,
	...
	s = &libcmd.Manager{
		Variables: map[string]interface{}{
			...
			libmonteur.VAR_TMP:     fx.workspace.Filesystem.TestTMPDir,
			...
			libmonteur.VAR_BASE:    fx.workspace.Filesystem.BaseDir,
...
func (fx *tester) _init() (err error) {
...
func (fx *tester) _reportError(err error) int {
	fx.logger.Error("%s %s\n", libmonteur.ERROR_TEST, err)
	fx.logger.Sync()
	fx.logger.Close()
	return STATUS_ERROR
}
